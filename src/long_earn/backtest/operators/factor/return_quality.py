...
class ReturnQualityParams(OperatorParams):
    field: str = "close"
    window: int = 20
    min_samples: int = 5
...
@operator
class ReturnQuality(Operator):
    """价格收益质量：滚动收益均值 / 波动（类 Sharpe）。

    仅使用 ``params.field`` 价格序列，**不是**基本面 ROE 因子。
    旧名 ``roe_quality`` 已废弃，见 ``OPERATOR_RENAMES``。
    """

    name: ClassVar[str] = "return_quality"
    ...
    params_cls: ClassVar[type[OperatorParams]] = ReturnQualityParams
    min_history: ClassVar[int] = 0

    def apply(self, panel: pl.DataFrame, params: OperatorParams) -> pl.Series:
        assert isinstance(params, ReturnQualityParams)
        p = params
