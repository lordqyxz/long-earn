...
def create_main_agent():
    ...
    def intent_analyze_node(state: State):
        ...
        routing_prompt = PromptTemplate(
            ...
            template=f"""请分析以下用户查询，确定用户意图并选择最合适的子图进行路由。   
...
只输出JSON，不要其他内容。""")
        ...
        try:
            ...
            def decide_route(user_query):
                if "策略" in user_query or "思路" in user_query or "投资策略" in user_query:
                    return ("strategy_rd", "关键词匹配: 策略相关")
                elif "股票" in user_query or "分析" in user_query or "公司" in user_query:
            ...
            try:
                # 清理响应内容，移除可能的前缀
                clean_response = routing_decision.replace('route：', '').strip()
                LOGGER.debug(f"清理后的响应: {repr(clean_response)}")
                # 尝试直接解析
                ...
                LOGGER.debug(f"JSON解析失败: {str(e)}，使用关键词匹配")
                # 解析失败，使用关键词匹配
                route, reason = decide_route(user_query)
            ...
            if "策略" in user_query:
                ...
                return {"route": "strategy_rd", "routing_reason": "关键词匹配: 策略相关"}
            elif "股票" in user_query:
                LOGGER.info("异常情况下使用关键词匹配: 股票相关")
                return {"route": "stock_analysis", "routing_reason": "关键词匹配: 股票相关"}
            else:
    ...
    def stock_analysis_node(state: State):
        ...
        result = stock_analysis_subgraph.invoke({"query": user_query})
    ...
    def summarize_node(state: State):
        ...
        summarize_prompt = PromptTemplate(
            input_variables=["strategy_result", "stock_analysis_result", "routing_reason"],
...
"""
        ...
        try:
            ...
            return {"summary": summary if summary else "处理过程中出现异常，请稍后再试。"}
