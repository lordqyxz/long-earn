...
from long_earn.core.storage import backtest_cache_path  # noqa: E402
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"审计库: {db}")
    print(f"目标回测: Momentum20Strategy, 2026-01-06 ~ 2026-07-08")
    ...
    for seq, ts, payload_json in md_rows:
    ...
    fills: list[dict] = []
    for seq, payload_json in fill_rows:
    ...
    for key, eng_val in metric_map:
        ...
        if key == "sortino":
            ...
            tol = max(1e-4, abs(eng_val) * 5e-3)  # 0.5% 相对容差
        ok = diff < tol
        checks.append(ok)
    ...
    print(f"  注：审计 MARKET_DATA 时点 ≠ equity_curve sync 时点（core.py:476 vs 524），")
    print(f"      交易后市值更新传导放大 ~0.3%，容差与段 A 一致取 0.5%")
    ...
    print(f"  注: trade_count 与 FILL 笔数维度可能不同（引擎统计调仓回合，FILL 统计单笔成交）")
    ...
    if open_positions:
        for s, q in list(open_positions.items())[:5]:
            print(f"    {s}: {q:.2f} 股")
    print(f"  结论: ✅ 所有 FILL 事件按 symbol 聚合后持仓变化可追溯")
